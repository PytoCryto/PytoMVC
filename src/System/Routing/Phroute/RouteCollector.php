...
use PytoMVC\System\Routing\Phroute\Exception\BadRouteException;
...
class RouteCollector implements RouteDataProviderInterface {
    ...
    public function before(array $methods, $callback) {
        $this->beforeAll = (object)['methods' => $methods, 'callback' => $callback];
    }
    ...
    public function route($name, array $args = null)
    {
        ...
        if (! isset($this->reverse[$name])) {
            throw new BadRouteException("Route name ({$name}) couldn't be found");
        }
     ...
     */
    public function group($filters, \Closure $callback)
    {
    ...
    public function getData()
    {
        if (empty($this->regexToRoutesMap)) {
            return new RouteDataArray($this->staticRoutes, [], $this->filters, $this->beforeAll);
        }

        return new RouteDataArray($this->staticRoutes, $this->generateVariableRouteData(), $this->filters, $this->beforeAll);
    }
